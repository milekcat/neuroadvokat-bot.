...
STATUS_TEXT = {"new": "Новое", "in_progress": "В работе", "closed": "Закрыто"}
...
async def cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Отменяет текущий процесс создания обращения."""
    user_id = str(update.effective_user.id)
...
async def take_ticket_action(query, context):
    ...
    if ticket_id in tickets_db:
        ...
        try:
            await context.bot.send_message(chat_id=int(client_user_id), text=f"✅ **Статус обновлен:** Ваше обращение №{ticket_id} принято в работу.", parse_mode='Markdown')
...
async def services_menu_action(query, context):
    ...
    if data == 'show_services_menu':
        ...
        await query.edit_message_text(SERVICE_DESCRIPTIONS[service_key], reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Создать обращение по этой теме", callback_data=f'order_{service_key}')], [InlineKeyboardButton("⬅️ К списку услуг", callback_data='show_services_menu')]]), parse_mode='Markdown')
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if current_state == 'in_ticket_chat':
        ...
        await update.message.reply_text(f"Приятно познакомиться, {name}!\n\nВашему обращению присвоен **номер {ticket_id}**.\n\nТеперь расскажите о вашей ситуации, отправляя сообщения, фото и файлы. Когда закончите, нажмите кнопку ниже.", reply_markup=ReplyKeyboardMarkup([["✅ Завершить и отправить обращение"]], one_time_keyboard=True, resize_keyboard=True), parse_mode='Markdown')
        return
...
async def reply_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if "ОБРАЩЕНИЕ №" not in replied_text: return
