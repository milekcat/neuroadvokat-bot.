...
async def order_action(query, context):
    ...
    # Устанавливаем состояние и сразу даем инструкции
    user_states[user_id] = {'state': 'collecting_data', 'category': CATEGORY_NAMES[category_key], 'user_name': user.full_name or user.first_name}
    save_json_data(user_states, USER_STATES_FILE, states_lock)

    await query.message.delete()
    await context.bot.send_message(
        chat_id=user_id,
        text=r"Отлично\! Теперь опишите вашу ситуацию и приложите все необходимые файлы\. Когда закончите, нажмите кнопку ниже\.",
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if current_state == 'in_ticket_chat':
        ...
        if 'active_ticket' not in current_state_data:
            ...
            name = current_state_data['user_name']
